...
## function to compute mean trait values for each clade in the phylogeny and
...
## SR_all: the number of taxa in each node.
## SR_mis: the number of non-missing taxa in each node (SR_all and SR_mis should match if the number
##         of species in the tree and that in samp is the same and there are no NAs in the table)
...
node.mean <- function(tree, samp, N) {
  ...
  sr.mis.node<-list() #create object to store the number of non-missing species found in each node.
  for(i in (n.tips+1):(n.tips+n.internal.nodes)){
    ...
    sr.mis.node[[i]]<-sum(!is.na(samp[rownames(samp) %in% spp.node, ]))
  }
  
  ## calculating mean expected values for each node:
  for(j in 1:N){
    samp.sel<-samp #create copy of samp
    set.seed(123+j) #for reproducibility
    samp.sel[!is.na(samp.sel)]<-sample(samp.sel[!is.na(samp.sel)], replace = F, prob = NULL) #shuffle values (equivalent to shuffling tips, but respecting NAs)
    
    for(i in (n.tips+1):(n.tips+n.internal.nodes)){
      node <- i
      spp.node <- tips(tree, node) # get taxa in node
      out[i-n.tips, j+1] <- mean(samp.sel[rownames(samp.sel) %in% spp.node, ], na.rm = T) # subset taxa and get the mean
    }
  }
  
  ## prepare data frame to store results:
  out2<-as.data.frame(matrix(ncol = 7, nrow = n.internal.nodes)) #create object to store results
  colnames(out2)<-c("Node", "SR_all", "SR_mis", "Obs", "Mean_Exp", "SD_Exp", "P_value") #set names of columns
  out2$Node<-seq((n.tips+1), (n.tips+n.internal.nodes)) #rownames are the names (numbers) of internal nodes
  out2$SR_all<-unlist(sr.node)
  out2$SR_mis<-unlist(sr.mis.node)
